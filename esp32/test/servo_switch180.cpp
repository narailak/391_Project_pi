...
static void cmd_cb(const void* msgin){
  ...
  const int angle = (int)m->data;

  // รับเฉพาะ 0 หรือ 180 ตามที่กำหนด
  if(angle == 0 || angle == 180){
    move_servo_angle(angle);
    last_angle = (int16_t)angle;
    publish_fb(last_angle);
  }
  // ค่าอื่นจะถูกเมิน (no-op)
}
...
static bool createEntities(){
  ...
  // Subscriber: /man/cmd_servo_switch180 (std_msgs/Int16)
  RCCHECK(rclc_subscription_init_default(
    &sub_cmd, &node,
    ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int16),
    "/man/cmd_servo_switch180"));

  // Publisher (best-effort): /man/cmd_servo_switch180/rpm (std_msgs/Int16)
  RCCHECK(rclc_publisher_init_best_effort(
    &pub_fb, &node,
    ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int16),
    "/man/cmd_servo_switch180/rpm"));
...
// -------- Arduino setup/loop --------
void setup(){
  // ต้องมีเพื่อส่ง micro-ROS ผ่าน Serial
  Serial.begin(115200);
  delay(50);
  ...
  move_servo_angle(0);  // เริ่มที่ 0° เงียบ ๆ ไม่สแกน
}
